...
def register():
    ...
    if form.validate_on_submit():
        user_register = User(username=form.username.data, email=form.email.data)
        user_register.set_password(form.password.data)
...
@login_required
def user(username):
    user_detail = User.query.filter_by(username=username).first_or_404()
    page = request.args.get('page', 1, type=int)
    posts = user_detail.posts.order_by(Post.timestamp.desc()).paginate(
        page, app.config['POSTS_PER_PAGE'], False
    )
    next_url = url_for('user', username=user_detail.username, page=posts.next_num) \
        if posts.has_next else None
    prev_url = url_for('user', username=user_detail.username, page=posts.prev_num) \
        if posts.has_prev else None
    return render_template(
        'user.html', title='Profil',
        user=user_detail,
        posts=posts.items,
        next_url=next_url, prev_url=prev_url
    )
